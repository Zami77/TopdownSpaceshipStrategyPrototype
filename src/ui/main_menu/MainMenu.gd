...
@onready var host_game_button: DefaultButton = $VBoxContainer/HostGameButton
@onready var join_game_button: DefaultButton = $VBoxContainer/JoinGameButton
...
enum Option { PLAY_GAME, SETTINGS, CREDITS, EXIT_GAME, HOST_GAME, JOIN_GAME }

func _ready():
	AudioManager.play_menu_theme()
	
	play_game_button.pressed.connect(_on_button_press.bind(Option.PLAY_GAME))
	host_game_button.pressed.connect(_on_button_press.bind(Option.HOST_GAME))
	join_game_button.pressed.connect(_on_button_press.bind(Option.JOIN_GAME))
